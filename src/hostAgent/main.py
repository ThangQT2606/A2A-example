...
async def get_response_from_agent(
    ...
    try:
        ...
        print(f'Timeout error in get_response_from_agent: {e}')
        traceback.print_exc()
        yield gr.ChatMessage(
            role='assistant',
            content='⏱️ **Timeout Error**: The request to the remote agent timed out. This may happen if the agent is taking too long to respond or is unavailable. Please try again or check if the remote agent is running.',
        )
        ...
        if 'timeout' in str(e).lower():
            error_message = '⏱️ **Timeout Error**: The request timed out. Please try again.'
        yield gr.ChatMessage(
            role='assistant',
...
async def main():
    ...
    with gr.Blocks(
        ...
        gr.ChatInterface(
            ...
            description='This assistant can help you to check weather and generate content.',
        )
